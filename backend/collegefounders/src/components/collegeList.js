        ...
        <>
          {props.stateFilter.length || props.courseFilter.length ? (
            <>
              ...
              {props.stateFilter.length ? <span>Statewise </span> : null}
              {props.courseFilter.length ? <span>Branchwise </span> : null}
            </>
          ) : null}
        </>
      </Breadcrumb>
      ...
      <Table
        ...
        dataSource={
          props.stateFilter.length
            ? props.data.filter((data, index) =>
                props.data[index].state
                  .toLowerCase()
                  .includes(props.stateFilter.toLowerCase())
              )
            : props.courseFilter.length
            ? props.data.filter((data, index) =>
                props.data[index].coursesOffered.includes(props.courseFilter)
              )
            : props.data
        }
        onChange={onChange}
